...
export default function EmergencyReportsList() {
  const { user } = useAuth();
  ...
  const isAmbulanceUser = userProfile?.role === 'ambulance';
  ...
  const handleCompleteEmergency = async (reportId: string) => {
    try {
      ...
      toast({
        ...
        description: "The emergency case has been marked as completed.",
        variant: "default",
      });
      ...
      <Table>
        <TableHeader>
          <TableRow>
            ...
            {isAmbulanceUser && <TableHead>Actions</TableHead>}
          </TableRow>
        </TableHeader>
        <TableBody>
          {reports.map((report) => (
